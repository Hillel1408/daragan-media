...
export default function MobileMenu({ activeMenu, setActiveMenu }: { activeMenu: boolean; setActiveMenu: (arg: (prev: boolean) => boolean) => void }) {
    const { t } = useTranslation();
    const { isMobile, isTablet, isDesktop } = useMatchMedia();

    const clickHandler = (e: any) => {
        if (!e.target.closest(".mobileMenuOpen") && isTablet) setActiveMenu((prev) => !prev);
    };
    ...
    return (
        <div
            className={classNames(
                "absolute sm:overflow-auto px-4 pb-[28px] pt-[13px] border duration-200 border-[#2b2b2d] rounded-[8px] right-0 top-0 bg-[#131316] z-20 w-[159px] flex flex-col translate-x-[14px] translate-y-[-16px] sm:translate-x-0 sm:translate-y-0 sm:w-full sm:top-[73px] sm:rounded-none sm:border-none sm:bg-[#111112] sm:px-0 sm:py-0 sm:fixed sm:bottom-0",
        ...
        >
            ...
            <div className="sm:bg-[#131316] sm:py-5 sm:px-5 sm:border-b sm:border-[#2b2b2d]">
                ...
                <Languages className="justify-end sm:justify-start" setActiveMenu={setActiveMenu} />
            </div>
            {isMobile && (
                <>
